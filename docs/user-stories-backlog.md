...
- Criar migração SQL `packages/database/migrations/003_seed_default_admin.sql`.
...
- [ ] A migração `003_seed_default_admin.sql` roda sem erros através de `bun run migrate`.
- [ ] Executar a migração múltiplas vezes não gera duplicidade nem erro de unicidade (idempotência garantida).
- [ ] A tabela `users` contém o registro do admin com `role = 3` e `deleted_at IS NULL`.
- [ ] A senha criptografada permite autenticação imediata pelo backend com `admin123`.
