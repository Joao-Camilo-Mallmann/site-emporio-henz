...
- **[Risco] Banco existente com resquícios de tabelas de desenvolvimento**:
  - _Mitigação_: Reset do banco via `docker compose down -v` assegura que o ambiente local e de CI execute as migrações a partir de um estado limpo. Além disso, `001_setup_extensions.sql` inclui `DROP TABLE IF EXISTS ... CASCADE` defensivo.
