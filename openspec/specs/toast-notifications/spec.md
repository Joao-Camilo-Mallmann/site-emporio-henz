# Capability: Toast Notifications

## Purpose

Fornecer um sistema flutuante não intrusivo de feedback visual e notificações (toasts) no frontend web (`apps/web`), garantindo persistência durante navegações de rota, harmonia com a tipografia do tema e disponibilidade tanto via Composition API (`useToast()`) quanto globalmente em templates (`$toast`).
